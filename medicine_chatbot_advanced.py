...
def main():
    ...
    st.markdown("### 🎯 PDF, Word, API সহ সব উৎস থেকে ওষুধের তথ্য খুঁজুন!")
    ...
    with st.sidebar:
        ...
        # ফাইল আপলোড সেকশন
        st.subheader("📁 ফাইল আপলোড")
        
        # PDF আপলোড
        pdf_file = st.file_uploader("PDF ফাইল আপলোড করুন", type=['pdf'], key="pdf_upload")
        if pdf_file:
            if st.button("📄 PDF যোগ করুন"):
                st.session_state.chatbot.add_pdf_file(pdf_file)
        
        # Word আপলোড
        word_file = st.file_uploader("Word ফাইল আপলোড করুন", type=['docx', 'doc'], key="word_upload")
        if word_file:
            if st.button("📝 Word যোগ করুন"):
                st.session_state.chatbot.add_word_file(word_file)
        
        # API সেটআপ
        st.subheader("🌐 API সংযোগ")
        api_url = st.text_input("API URL:", placeholder="https://api.example.com/medicines")
        api_key = st.text_input("API Key (ঐচ্ছিক):", type="password", placeholder="your-api-key")
        
        col1, col2 = st.columns(2)
        with col1:
            if st.button("🔗 API যোগ করুন") and api_url:
        ...
        if st.session_state.chatbot.pdf_data or st.session_state.chatbot.word_data or st.session_state.chatbot.api_data:
    ...
    search_type = st.radio(
        ...
        ["📊 Excel ডেটা", "🌐 সব উৎস (PDF/Word/API)"],
        horizontal=True
    )
    ...
    if search_button and user_query:
        ...
        if search_type == "📊 Excel ডেটা":
            # শুধু Excel ডেটা থেকে সার্চ
