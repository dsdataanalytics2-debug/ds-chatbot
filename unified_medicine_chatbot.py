...
# -*- coding: utf-8 -*-
"""
💊 ইউনিফাইড মেডিসিন চ্যাটবট - সব ফাইল এক জায়গায় আপলোড
...
def main():
    st.set_page_config(
        page_title="💊 ইউনিফাইড মেডিসিন চ্যাটবট",
        page_icon="💊",
        layout="wide",
    ...
    # হেডার
    st.markdown('<h1 class="main-header">💊 ইউনিফাইড মেডিসিন চ্যাটবট</h1>', unsafe_allow_html=True)
    st.markdown("### 🎯 সব ফাইল এক জায়গায় আপলোড করুন - PDF, Word, Excel, API!")
    
    # চ্যাটবট ইনিশিয়ালাইজ করুন
    if 'chatbot' not in st.session_state:
        st.session_state.chatbot = UnifiedMedicineChatbot('medicine_data.xlsx')
    ...
    with st.sidebar:
        ...
        if st.session_state.chatbot.uploaded_files:
            ...
            for file_item in st.session_state.chatbot.uploaded_files:
                ...
                filename = file_item.get('filename', file_item.get('url', 'Unknown'))
                st.info(f"{source_icon} {file_item['source']}: {filename} ({file_item['upload_time']})")
...
• ইংরেজি বা বাংলা উভয় ভাষায় চেষ্টা করুন"""
    ...
    with col3:
        st.markdown("**🌐 ইউনিফাইড আপলোড**")
        st.markdown("সব ফাইল এক জায়গায়")
